...
export function TransactionItemsEditor({ materials, items, onChange }: { materials: Material[]; items: TransactionItem[]; onChange: (items: TransactionItem[]) => void }) {
  const update = (id: string, patch: Partial<TransactionItem>) => onChange(items.map((item) => item.id === id ? { ...item, ...patch } : item));
  const remove = (id: string) => onChange(items.filter((item) => item.id !== id));
  const add = () => {
    const first = materials[0];
    if (!first) return;
    onChange([...items, { id: uid('item'), materialId: first.id, quantity: 1, rate: first.standardRate, taxRate: first.taxRate }]);
  };
    ...
    <div className="items-editor">
      ...
      <div className="items-table-wrap">
        <table className="items-table">
          <thead><tr><th>Material</th><th>Qty</th><th>Rate</th><th>Tax %</th><th>Amount</th><th /></tr></thead>
          <tbody>
            {items.map((item) => {
              return (
                <tr key={item.id}>
                  <td>
                    <select value={item.materialId} onChange={(event) => {
                      ...
                      update(item.id, { materialId: event.target.value, rate: selected?.standardRate ?? item.rate, taxRate: selected?.taxRate ?? item.taxRate });
                    }}>
                  ...
                  <td><input type="number" min="0" step="0.01" value={item.taxRate} onChange={(event) => update(item.id, { taxRate: Number(event.target.value) })} /></td>
                  <td className="amount-cell">₹{(item.quantity * item.rate * (1 + item.taxRate / 100)).toLocaleString('en-IN', { maximumFractionDigits: 2 })}</td>
