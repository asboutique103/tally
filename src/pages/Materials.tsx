...
const emptyMaterial = (): Material => ({ id: uid('mat'), code: '', name: '', category: '', unit: 'Nos', standardRate: 0, taxRate: 18, reorderLevel: 0, openingStock: 0, location: '', createdAt: new Date().toISOString() });
          ...
          <div className="table-scroll">
            <table className="data-table">
              ...
              <tbody>{filtered.map((item) => (
                <tr key={item.id}>
                  <td><strong>{item.name}</strong><span>{item.code} · {item.location || 'No rack'}</span></td>
