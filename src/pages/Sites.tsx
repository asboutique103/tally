...
import { cleanText, compactPhone, hasDuplicate, isFilled, isValidIndianPhone } from '../lib/validation';
...
export function Sites() {
  ...
  const submit = (event: FormEvent) => {
    ...
    if (!isFilled(next.code) || !isFilled(next.name) || !isFilled(next.clientName) || !isFilled(next.location) || !isFilled(next.phone) || !isFilled(next.startDate)) {
      setError('Site code, project name, client, location, phone and start date are mandatory.');
      return;
    }
  ...
  return (
    <div className="page-stack">
      <PageHeader
        ...
        description="Track construction locations, clients and material consumption."
        actions={(
          <>
      ...
      <Modal open={open} title={data.sites.some((site) => site.id === draft.id) ? 'Edit site' : 'Create site'} subtitle="Sites receive issued materials and can have client billing." onClose={() => setOpen(false)} wide>
        <form className="form-stack" onSubmit={submit}>
          ...
          <div className="form-grid two">
            ...
            <label><span>Expected end date</span><input type="date" value={draft.expectedEndDate} onChange={(event) => setDraft({ ...draft, expectedEndDate: event.target.value })} /></label>
          </div>
